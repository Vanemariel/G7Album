...
const config = {
    individualItem3: '.album-item3',
     // class of individual item
    carouselWidth: 1000, // in px
    carouselId3: '#album-rotator3',
    // carousel selector
     // carousel selector
 
     carouselHolderId3: '#album-rotator-holder3',// carousel should be <div id="carouselId3"><div id="carouselHolderId3">{items}</div></div>
    colors: [
...
document.addEventListener("DOMContentLoaded", function(e) {
    createWave(config.individualItem3, config.colors);

    // Get items
    const el = document.querySelector(config.individualItem3);
    ...
    document.querySelectorAll(config.carouselId3).forEach(function(item) { 
    ...
    document.querySelectorAll(config.carouselId3).forEach(function(item) {
        item.addEventListener('pointerdown', function(e) {
            ...
            currentDelta = parseFloat(item.querySelector(config.carouselHolderId3).style.transform.split('translateX(')[1]) || 0;
        }); 
    });
    ...
    const scrollCarousel = function(change, currentDelta, selectedItem) {
        ...
        let elLength = selectedItem.querySelectorAll(config.individualItem3).length - numberThatFit;
        if(newDelta <= 0 && newDelta >= -elWidth * elLength) {
            selectedItem.querySelector(config.carouselHolderId3).style.transform = `translateX(${newDelta}px)`;
        } else {
            if(newDelta <= -elWidth * elLength) {
                selectedItem.querySelector(config.carouselHolderId3).style.transform = `translateX(${-elWidth * elLength}px)`;
            } else if(newDelta >= 0) {
                selectedItem.querySelector(config.carouselHolderId3).style.transform = `translateX(0px)`;
            }
        }
    }
    ...
    document.body.addEventListener('pointermove', function(e) {
        if(mousedown == true && typeof selectedItem !== "undefined") {
            ...
            document.querySelectorAll(`${config.carouselId3} a`).forEach(function(item) {
    ...
    document.querySelectorAll(config.carouselId3).forEach(function(item) {
        ...
        item.addEventListener('wheel', function(e) {
            if(trigger !== 1) {
                ...
                let currentDelta = parseFloat(item.querySelector(config.carouselHolderId3).style.transform.split('translateX(')[1]) || 0;
