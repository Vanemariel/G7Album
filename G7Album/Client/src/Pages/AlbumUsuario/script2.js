...
const config = {
    individualItem2: '.album-item2',
     // class of individual item
    carouselWidth: 1000, // in px
    carouselId2: '#album-rotator2',
    // carousel selector
     // carousel selector
 
     carouselHolderId2: '#album-rotator-holder2',// carousel should be <div id="carouselId2"><div id="carouselHolderId2">{items}</div></div>
    colors: [
...
document.addEventListener("DOMContentLoaded", function(e) {
    createWave(config.individualItem2, config.colors);

    // Get items
    const el = document.querySelector(config.individualItem2);
    ...
    document.querySelectorAll(config.carouselId2).forEach(function(item) {
        item.addEventListener('pointerdown', function(e) {
            ...
            currentDelta = parseFloat(item.querySelector(config.carouselHolderId2).style.transform.split('translateX(')[1]) || 0;
        }); 
    });
    ...
    const scrollCarousel = function(change, currentDelta, selectedItem) {
        ...
        let elLength = selectedItem.querySelectorAll(config.individualItem2).length - numberThatFit;
        if(newDelta <= 0 && newDelta >= -elWidth * elLength) {
            ...
            if(newDelta <= -elWidth * elLength) {
                selectedItem.querySelector(config.carouselHolderId2).style.transform = `translateX(${-elWidth * elLength}px)`;
            } else if(newDelta >= 0) {
    ...
    document.querySelectorAll(config.carouselId2).forEach(function(item) {
        ...
        item.addEventListener('wheel', function(e) {
            if(trigger !== 1) {
                ...
                let currentDelta = parseFloat(item.querySelector(config.carouselHolderId2).style.transform.split('translateX(')[1]) || 0;
