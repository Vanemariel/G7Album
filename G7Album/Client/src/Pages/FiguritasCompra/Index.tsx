//import { useEffect } from "react";
//import { FormLogin } from "./Components/FormLogin/FormLogin";

export const FiguritasCompra: React.FC = () => {


    /// METODOS


    return (
        <div id="m">
            <div id="album-rotator">
                <h1>FIGURITAS DE DEPORTES</h1>
                <div id="album-rotator-holder">
                    <a target="_top" class="album-item" href="https://twitter.com/smpnjn">
                        <span class="album-details">
                            <span class="icon"><i class="far fa-at"></i> GroupAlbumesEdition</span>
                            <span class="title">Click</span>
                            <span class="subtitle">Figus Futbol</span>
                            <span class="subtext">Copa Libertadores<br>Copa America<br>Champios League</span>
                            </span>
                    </a>
                    <a target="_top" class="album-item" href="https://fjolt.com/article/apple-cards-webl-gl-javascript">
                        <span class="album-details">
                            <span class="icon"><i class="far fa-at"></i> GroupAlbumesEdition</span>
                            <span class="title">Click</span>
                            <span class="subtitle">Figus Tenis</span>
                            <span class="subtext">Wimledon<br>Rollan Garros<br>Us Open</span>
                            </span>
                    </a>
                    <a target="_top" class="album-item" href="https://twitter.com/smpnjn">
                        <span class="album-details">
                            <span class="icon"><i class="far fa-at"></i> GroupAlbumesEdition</span>
                            <span class="title">Click</span>
                            <span class="subtitle">Figus Basket</span>
                            <span class="subtext">Liga Endesa<br>NBA<br>La Liga Argentina</span>
                            </span>
                    </a>
                    <a target="_top" class="album-item" href="https://twitter.com/smpnjn">
                        <span class="album-details">
                            <span class="icon"><i class="far fa-at"></i> GroupAlbumesEdition</span>
                            <span class="title">Click para ver las figuritas</span>
                            <span class="subtitle">Figus Rugby</span>
                            <span class="subtext">Nationas Rugby League<br>Super League<br>The Rugby ChampionsShip</span>
                            </span>
                    </a>
                </div>
            </div>

            <!-- 2 bloque-->

            <div id="album-rotator2">
                <h1>FIGURITAS DE DISNEY</h1>
                <div id="album-rotator-holder2">
                    <a target="_top" class="album-item2" href="https://twitter.com/smpnjn">
                        <span class="album-details2">
                            <span class="icon"><i class="far fa-at"></i> GroupAlbumesEdition</span>
                            <span class="title">Click</span>
                            <span class="subtitle">Figus Monster Inc</span>
                            <span class="subtext">Monster Inc<br>Monster University</span>
                        </span>
                    </a>
                    <a target="_top" class="album-item2" href="https://fjolt.com/article/apple-cards-webl-gl-javascript">
                        <span class="album-details2">
                            <span class="icon"><i class="far fa-at"></i> GroupAlbumesEdition</span>
                            <span class="title">Click</span>
                            <span class="subtitle">Figus High school Musical</span>
                            <span class="subtext">High school Musical<br>High school Musical 2</span>
                        </span>
                    </a>
                    <a target="_top" class="album-item2" href="https://twitter.com/smpnjn">
                        <span class="album-details2">
                            <span class="icon"><i class="far fa-at"></i> GroupAlbumesEdition</span>
                            <span class="title">hielo</span>
                            <span class="subtitle">Figus </span>
                            <span class="subtext">La Hera de hielo 1<br>La era de hielo 2<br>La era de hielo 3</span>
                            </span>
                    </a>
                    <a target="_top" class="album-item2" href="https://twitter.com/smpnjn">
                        <span class="album-details2">

                            <span class="title">Avatar</span>
                            <span class="subtitle">Figus Avatar</span>
                            <span class="subtext">Avatar<br>Avatar 2</span>
                        </span>
                    </a>
                </div>
            </div>
            <!--tercer bloque-->

            <div id="album-rotator3">
                <h1>FIGURITAS DE ANIME</h1>
                <div id="album-rotator-holder3">
                    <a target="_top" class="album-item3" href="https://twitter.com/smpnjn">
                        <span class="album-details3">
                            <span class="icon"><i class="far fa-at"></i> GroupAlbumesEdition</span>
                            <span class="title">Click</span>
                            <span class="subtitle">Figus Dragon Ball Z</span>
                            <span class="subtext">Dragon Ball Z<br>Dragon Ball GT</span>
                        </span>
                    </a>
                    <a target="_top" class="album-item3" href="https://fjolt.com/article/apple-cards-webl-gl-javascript">
                        <span class="album-details3">
                            <span class="icon"><i class="far fa-at"></i> GroupAlbumesEdition</span>
                            <span class="title">Click</span>
                            <span class="subtitle">Figus Naruto</span>
                            <span class="subtext">Naruto<br>Naruto Shippuden<br>Naruto Next Generation</span>
                            </span>
                    </a>
                    <a target="_top" class="album-item3" href="https://twitter.com/smpnjn">
                        <span class="album-details3">
                            <span class="icon"><i class="far fa-at"></i> GroupAlbumesEdition</span>
                            <span class="title">Click</span>
                            <span class="subtitle">Figus Los Caballeros de Zodiaco</span>
                            <span class="subtext">Saint Seiya the Lost Canvas<br>Batalla de Poseidon<br>Batalla de Asgard</span>
                            </span>
                    </a>
                    <a target="_top" class="album-item3" href="https://twitter.com/smpnjn">
                        <span class="album-details3">
                            <span class="icon"><i class="far fa-at"></i> GroupAlbumesEdition</span>
                            <span class="title">Click para ver las figuritas</span>
                            <span class="subtitle">Figus da Ataque a los Titanes</span>
                            <span class="subtext">Temprada 1<br>Temporada 2</span>
                        </span>
                    </a>
                </div>
            </div>
    )
}